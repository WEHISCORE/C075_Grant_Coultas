...
#### Mean-variance plots stratified by genotype

In a meeting on 2020-05-01, Anne asked for a version of Figure \@ref(fig:trendplots) by genotype.
Anne was interested to know if there is a difference (due to HBO1-induced global gene expression changes).
Figure \@ref(fig:trendplots-by-genotype) does this and shows no strong genotype-specific differences.

```{r trendplots-by-genotype, fig.cap = "Variance of normalized log-expression values for each gene in each plate and genotype, plotted against the mean log-expression. The blue line represents the mean-dependent trend fitted to the variances of the spike-in transcripts (red).", results = "hide", fig.asp = 1}
dec_genotype <- modelGeneVarWithSpikes(
  sce,
  "ERCC",
  block = paste0(sce$plate_number, ".", sce$genotype))

xlim <- c(0, max(sapply(dec_genotype$per.block, function(x) max(x$mean))))
ylim <- c(0, max(sapply(dec_genotype$per.block, function(x) max(x$total))))
par(mfrow = c(3, 3))
blocked_stats <- dec_genotype$per.block
for (i in colnames(blocked_stats)) {
  current <- blocked_stats[[i]]
  plot(
    current$mean,
...
- Merge `LC392`, and `LC280` (which all have both a mutant and control mouse)
...
```{r beforeplot-umap, fig.cap = "UMAP plot of the data set before MNN correction. Each point represents a cell with the colour explained in the legend.", fig.asp = 0.8}
set.seed(572968)
library(uwot)
...
```{r afterplot-umap, fig.cap = "UMAP plot of the data set after MNN correction. Each point represents a cell with the colour explained in the legend.", fig.asp = 0.8}
set.seed(55788)
...
```{r afterplot-umap-facetted, fig.cap = "UMAP plot of the data set after MNN correction, highlighting the effect of plate number. Each panel highlights a particular plate number with cells coloured if they are from the featured plate number or grey otherwise.", fig.asp = 5 / 4}
...
```{r afterplot-umap-genotype, fig.cap = "UMAP plot of the data set after MNN correction, highlighting the effect of genotype. Each panel highlights a particular genotype with cells highlighted by genotype or grey if they are not from the featured genotype.", fig.asp = 1 / 2}
...
```{r afterplot-umap-sex, fig.cap = "UMAP plot of the data set after MNN correction, highlighting the effect of sex. Each panel highlights a particular genotype with cells highlighted by genotype or grey if they are not from the featured sex", fig.asp = 1 / 2}
...
```{r afterplot-umap-mouse, fig.cap = "UMAP plot of the data set after MNN correction, highlighting the effect of sex. Each panel highlights a particular genotype with cells highlighted by genotype or grey if they are not from the featured sex", fig.asp = 1}
