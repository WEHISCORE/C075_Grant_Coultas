...
```{r trendplots, fig.cap = "Variance of normalized log-expression values for each gene in each plate, plotted against the mean log-expression. The coloured line represents the mean-dependent trend fitted to the variances of the spike-in transcripts (red).", results = "hide", fig.asp = 1 / 2, layout = "l-page"}
...
**TODO:** Re-visit above if changing HVGs

**TODO:** Does sex explain separation in UMAP (preliminary results say, no).
...
However, there is a notable exception of a subpopulation of cells on plats `LC294` and `LC358`.
...
ggplot(aes(x = x, y = y), data = corrected_umap) +
  ...
  geom_point(aes(colour = batch), alpha = 1, size = 1) +
  scale_fill_manual(values = batch_colours, name = "batch") + 
  scale_colour_manual(values = batch_colours, name = "batch") +
...
We expect to see the circulating hybrid cells in different locations on the UMAP to the CD45+ control cells, as this would indicate that there are differences between the cell types. 
If this was the case, we would see grey areas on each of the UMAP plots which is indicative of that cell type not being in that area of the UMAP. 
Since both cell types occupy all areas of the UMAP, at this stage we cannot identify differences between the two cell types. 

Figures \@ref(fig:umap-with-facs) and \@ref(fig:tsne-with-facs) overlay the index sorting data on the UMAP and TSNE plots respectively. 
...
- [ ] What are the sex of the mice? Can estimate from `plotExpression(sce, "Xist", x = "plate_number.mouse")` and have asked Zoe for info.
