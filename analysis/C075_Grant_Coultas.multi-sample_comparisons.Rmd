...
Finally, we perform 'rotation gene set tests' using the `fry()` function in the `r BiocStyle::Biocpkg("edgeR")` [@wu2010roast].
...
<aside>
The supplied gene list does not include log-fold changes, P-values, or any other form of directionality or ranking of the genes and therefore cannot be used to create a barcode plot.
</aside>
...
- The `PValue` column gives the two-sided p-value for testing whether the set is DE as a whole, either up or down. This column is appropriate when genes in the set are expected to be co-regulated, all or most changing expression in the same direction.
- The `PValue.Mixed` column gives a p-value for testing whether genes in the set tend to be DE, without regard to direction. This column is appropriate when genes in the set are not necessarily co-regulated or may be regulated in different directions for the contrast in question. 
...
knitr::kable(fr, caption = "Results of `fry()` analysis.")
```
