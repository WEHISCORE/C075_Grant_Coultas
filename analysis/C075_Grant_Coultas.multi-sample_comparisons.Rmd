...
**TODO** Minimise redundancy with DE analysis section
...
This is lower than generally recommended^[E.g., @crowell2019discovery note that "$\approx100$ cells is sufficient to reach decent performance" with pseudobulk DE methods and that "there is a sizable gain in performance in going from 50 to 100 cells" per label per sample.] but here we are limited by the small numbers of cells/mouse that were processed.
...
```{r degs-ignoring-clusters-heatmap, fig.cap = "Heatmap of pseudobulk-level log-expression values normalized to the mean of all samples in the filtered data; rows correspond to genes, columns to label-sample combinations. Included is the union of DEGs detected (FDR < `r fdr`).", fig.asp = 1.6}
o <- order(summed_filt$label, summed_filt$genotype, summed_filt$sample)
