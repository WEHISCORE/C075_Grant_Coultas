...
# NOTE: The default `min.count = 10` excludes the one interesting label 
#       (`cluster_5`).
keep <- filterByExpr(y_ab, group = y_ab$samples$genotype, min.count = 5)
y_ab <- y_ab[keep, ]
```
...
In general, there seems to be little differential expression that is associated with genotype, except within `clusters_3_4_6`.

<aside>
A PDF version of Figure \@ref(fig:degs-heatmap) is available in [`output/DEGs/degs-heatmap.pdf`](../output/DEGs/degs-heatmap.pdf).
</aside>
...
pheatmap(
  ...
  fontsize = 8,
  gaps_col = cumsum(runLength(Rle(summed_filt$label[o]))),
  filename = file.path(deg_dir, "degs-heatmap.pdf"),
  width = 7,
  height = 10)
```
...
de_results_ignoring_clusters <- pseudoBulkDGE(
  summed_filt, 
  sample = summed_filt$sample,
  label = summed_filt$label,
  design = design,
...
<aside>
A PDF version of Figure \@ref(fig:degs-ignoring-clusters-heatmap) is available in [`output/DEGs/degs-ignoring-clusters-heatmap.pdf`](../output/DEGs/degs-ignoring-clusters-heatmap.pdf) and a version showing just the top-60 DEGs by FDR in [`output/DEGs/degs-ignoring-clusters-heatmap-top-60.pdf`](../output/DEGs/degs-ignoring-clusters-heatmap-top-60.pdf).
</aside>
...
pheatmap(
  ...
  breaks = seq(-max(abs(mat)), max(abs(mat)), length.out = 101),
  fontsize = 6,
  filename = file.path(deg_dir, "degs-ignoring-clusters-heatmap.pdf"),
  width = 7,
  height = 12)

# PDF version retricted to top-60 genes by FDR (requested by Zoe in email on 
# 2020-09-19)
all_features <- lapply(
  de_results, 
  function(x) head(rownames(x[order(x$FDR), ]), 60))
features <- unique(unlist(all_features))
mat <- logcounts(summed_filt)[features, ]
mat <- mat - rowMeans(mat)
pheatmap(
  ...
  breaks = seq(-max(abs(mat)), max(abs(mat)), length.out = 101),
  fontsize = 8,
  filename = file.path(deg_dir, "degs-ignoring-clusters-heatmap-top-60.pdf"),
  width = 7,
  height = 12)
```
...
```{r}
is_de <- decideTestsPerLabel(de_results_ignoring_clusters, threshold = fdr)
summarizeTestsPerLabel(is_de) %>%
  knitr::kable(
    caption = "Number of DEGs per label. Each row corresponds to a label and each column corresponds to the number of downregulated genes (`-1`), the number of non-differentially expressed genes (`0`), the number of upregulated genes (`1`), and the number of genes not tested (`NA`).")
```
...
```{r, message = FALSE}
# Create outputs
createDEGOutputs(outdir = deg_dir, de_results = de_results_ignoring_clusters)
```

Please see [`output/DEGs/`](../output/DEGs/) for spreadsheets of these DEG lists, including results of GO and KEGG analyses using the `goana()` and `kegga()` functions from the `r BiocStyle::Biocpkg("limma")` package^[Please note that these results do not take into account the directionality of the log-fold change.], and [`output/glimma-plots`](../output/glimma-plots/) for interactive `r BiocStyle::Biocpkg("Glimma")` plots of the differential expression results.
The `r BiocStyle::Biocpkg("Glimma")` plots show the pseudobulk data for that label on the left-hand side and the single-cell data for that label on the right-hand side.
The single-cell data are plotted per sample using abbreviated sample labels, e.g., `C.LC280` means "control mouse on plate LC280", `M.LC280` means "mutant mouse on plate LC280", etc.

### Futher gene set testing

```{r}
# NOTE: Have to essentially re-do the analysis because `de_results` does not 
#       contain the intermediate outputs (e.g., DGEList). This will not be 
#       necessary from BioC 3.12 because scran::pseudoBulkDGE() will include 
#       these intermediate outputs.
y <- DGEList(
  counts(summed),
  samples = colData(summed),
  genes = flattenDF(rowData(summed)))
y <- y[, y$samples$ncells >= min_ncells]
...
As noted above, the GO and KEGG results computed so far have been non-directional (i.e. they don't incorporate the direction of the log-fold change).
With a bit more work we can incorporate the log-fold change to create more specific GO and KEGG analyses.
Please see the CSV files with "directional" in the file name in [`output/DEGs/`](../output/DEGs/) for spreadsheets of directional GO and KEGG results^[Note that the number of `Up` and `Down` genes may not match the number of entires in `upGeneSymbols` and `downGeneSymbol` in these spreadsheets. This is because we have performed our analysis using the Ensembl gene ID as the features whereas the GO and KEGG analyses using Entrez gene IDs. There is no one-to-one mapping between the Ensembl gene IDs, Entrez gene IDs, and NCBI gene symbols, which leads to the slight variations of the gene numbers in these spreadsheets.].

```{r}
# Run GO analysis
# NOTE: Features map to multiple ENTREZID, so just select the first.
geneid <- sapply(strsplit(res$genes$NCBI.ENTREZID, ";"), "[[", 1)
go <- goana(res, coef = "Mutant", species = "Mm", geneid = geneid)
...
go_symbol_df <- dplyr::left_join(
  go_symbol_df, 
  as.data.frame(topTags(res, n = Inf))[, c("logFC", "FDR", "NCBI.SYMBOL")],
...
go <- dplyr::left_join(
  ...
  go_symbol_df %>%
    ...
    dplyr::summarise(
      upGeneSymbols = 
        paste(unique(SYMBOL[which(logFC > 0 & FDR < fdr)]), collapse = ", "),
      downGeneSymbols = 
        paste(unique(SYMBOL[which(logFC < 0 & FDR < fdr)]), collapse = ", ")),
  by = c("GO" = "GOALL"))
...
gzout <- gzfile(
  description = file.path(deg_dir, "ignoring_clusters.directional.GO.csv.gz"),
  open = "wb")
write.csv(
  go,
  gzout,
...
# NOTE: Features map to multiple ENTREZID, so just select the first.
geneid <- sapply(strsplit(res$genes$NCBI.ENTREZID, ";"), "[[", 1)
kegg <- kegga(res, coef = "Mutant", species = "Mm", geneid = geneid)

# Get gene symbols associated with each KEGG pathway.
library(org.Mm.eg.db)
kegg_symbol_df <- select(
  org.Mm.eg.db,
  # NOTE: The org.MM.eg.db does not use the 'path:mmu'-prefix.
  keys = sub("path:mmu", "", rownames(kegg)),
  columns = "SYMBOL", 
  keytype = "PATH")
...
kegg <- dplyr::left_join(
  kegg %>% 
    ...
    dplyr::mutate(PATH = sub("path:mmu", "", PATH)),
  kegg_symbol_df %>%
    dplyr::group_by(PATH) %>%
    dplyr::summarise(
      upGeneSymbols = 
  ...
  by = c("PATH" = "PATH"))
kegg <- kegg %>%
  dplyr::mutate(PATH = paste0("path:mmu", PATH)) %>%
  tibble::column_to_rownames("PATH")

gzout <- gzfile(
  description = file.path(deg_dir, "ignoring_clusters.directional.KEGG.csv.gz"),
  open = "wb")
write.csv(
  kegg,
  gzout,
...
In addition to the GO and KEGG analyses, we perform 'rotation gene set tests' using the `fry()` function in the `r BiocStyle::Biocpkg("edgeR")` [@wu2010roast].
This tests whether a set of genes is DE, assessing the set of genes as a whole. 
Gene set tests consider all the genes in the specified set and do not depend on any pre-emptive significance cut off.
The set of genes can be chosen to be representative of any pathway or phenotype of interest.
Zoe sent me such a set of genes (see [`data/zoes_marker_genes/20200728 barcode plot gene lists.xlsx`](../data/zoes_marker_genes/20200728 barcode plot gene lists.xlsx)).

<aside>
The supplied gene list does not include log-fold changes, P-values, or any other form of directionality or ranking of the genes.
</aside>

```{r}
sheet_names <- readxl::excel_sheets(
  here("data", "zoes_marker_genes", "20200728 barcode plot gene lists.xlsx"))
barcode_plot_genes <- lapply(sheet_names, function(sheet) {
  unlist(
    readxl::read_excel(
      here("data", "zoes_marker_genes", "20200728 barcode plot gene lists.xlsx"),
      sheet = sheet,
...
names(barcode_plot_genes) <- sub("^[0-9]\\. ", "", sheet_names)
```

The results of the `fry()` analysis are shown below.

```{r}
index <- lapply(barcode_plot_genes, function(x) {
  rownames(y) %in% x
})
fr <- fry(
  y = y,
...
knitr::kable(
  ...
  caption = "Results of `fry()` analysis. The row name provides the name of the gene set. The `NGenes` column provides the number of genes in each set. The `Direction` column indicates the net direction of change. The `PValue` column gives the two-sided p-value for testing whether the set is DE as a whole, either up or down; this column is appropriate when genes in the set are expected to be co-regulated, all or most changing expression in the same direction. The `PValue.Mixed` column gives a p-value for testing whether genes in the set tend to be DE, without regard to direction; This column is appropriate when genes in the set are not necessarily co-regulated or may be regulated in different directions for the contrast in question.")
```

Figure \@ref(fig:barcode-plot) visualises the enrichment of the each gene set using a barcode enrichment plot.

<aside>
A PDF version of Figure \@ref(fig:barcode-plot) is available in [`output/DEGs/barcode_plots.pdf`](../output/DEGs/barcode_plots.pdf).
</aside>

```{r barcode-plot, fig.cap = "Barcode plot of the gene set supplied by Zoe. An enrichment line shows the relative enrichment of the vertical bars in each part of the plot is displayed.", fig.asp = 3 / 2, results = "hide"}
par(mfrow = c(3, 2))
lapply(names(index), function(n) {
  barcodeplot(res$table$logFC, index = index[[n]], main = n)
})
par(mfrow = c(1, 1))
pdf(here("output", "DEGs", "barcode_plots.pdf"))
lapply(names(index), function(n) {
  barcodeplot(res$table$logFC, index = index[[n]], main = n)
})
dev.off()
...
Finally, Figure \@ref(fig:barcode-plot-ignoring-clusters-degs) visualises the enrichment of the `ignoring_clusters` DEGs amongst the within-label DE results using barcode enrichment plots.

<aside>
A PDF version of Figure \@ref(fig:barcode-plot-ignoring-clusters-degs) is available in [`output/DEGs/barcode_plots_ignoring_clusters_degs.pdf`](../output/DEGs/barcode_plots_ignoring_clusters_degs.pdf).
</aside>

```{r barcode-plot-ignoring-clusters-degs, fig.cap = "Barcode plot of the `ignoring_clusters` DEGs amongst the within-label DE analyses. An enrichment line shows the relative enrichment of the vertical bars in each part of the plot is displayed.", fig.asp = 3 / 2, results = "hide"}
ignoring_clusters_degs <- de_results_ignoring_clusters[[1]][
  which(de_results_ignoring_clusters[[1]]$FDR < 0.05), ]
barcode_plot_genes <- split(
  rownames(ignoring_clusters_degs),
  sign(ignoring_clusters_degs$logFC))
...
lapply(names(de_results), function(n) {
  ...
  barcodeplot(res$logFC, index = index[["1"]], index2 = index[["-1"]], main = n)
})
par(mfrow = c(1, 1))
pdf(here("output", "DEGs", "barcode_plots_ignoring_clusters_degs.pdf"))
lapply(names(de_results), function(n) {
  res <- de_results[[n]]
  index <- lapply(barcode_plot_genes, function(x) {
    rownames(res) %in% x
