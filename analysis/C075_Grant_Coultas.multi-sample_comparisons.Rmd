...
This directory also contains spreadsheets of results from GO and KEGG analyses, using the `goana()` and `kegga()` functions from the `r BiocStyle::Biocpkg("limma")` package, of cluster-specific DEGs.
Please note that these results do not take into account the directionality of the log-fold change.
...
Please see [`output/DEGs/`](../output/DEGs/) for spreadsheets, including results of GO and KEGG analyses, and interactive `r BiocStyle::Biocpkg("Glimma")` plots of these DEG lists.
...
Zoe sent me such a set of genes, shown below.
...
```{r}
barcode_plot_genes <- readxl::read_excel(
  here("data/zoes_marker_genes/20200728 barcode plot gene lists.xlsx"),
  col_names = FALSE)
colnames(barcode_plot_genes) <- "Gene"
DT::datatable(
  barcode_plot_genes,
  caption = "Gene list supplied by Zoe for use in gene set testing (2020-07-28).")
```
...
#       contain the intermediate outputs (e.g., DGEList).
