...
**TODO** UP TO HERE. Repeat analysis from Cooney but in style of above.
...
1. [`pval.type = "all"`: Consolidating with DE against *any* other cluster]
2. [`pval.type = "any"`: Consolidating with DE against *all* other clusters]
...
#### `pval.type = "all"`: Consolidating with DE against *any* other cluster
...
The set of genes with `Top`$<= 1$ will contain the top gene from each pairwise comparison to every other cluster.
...
The combined p-value is computed by applying Simes' method to all p-values. Neither of these values are directly used for ranking and are only reported for the sake of the user.

#### `pval.type = "any"`: Consolidating with DE against *all* other clusters
...
#### `direction = "up"` and `pval.type = "some"`
...
#### `direction = "any"` and `pval.type = "any"`
...
As an example of the information available in these gene lists, Figure \@ref(fig:t-up-all-heatmap) is a heatmap showing the expression of the top-10 marker genes for each cluster from the `t_up_all` analysis (see [`direction = "up"` and `pval.type = "up"`]). 
...
```{r t-up-all-heatmap, fig.wide = TRUE, fig.cap = "Heatmap of log-expression values for top-10 upregulated cluster-specific marker genes. Each column is a sample, each row a gene. Note that some clusters have as few as zero upregulated cluster-specific marker genes.", fig.asp = 1.5}
direction <- "up"
pval.type <- "all"
min.prop <- NULL
...
- [ ] Create versions of heatmaps using `logcounts` and `reconstructed` values

# Summary

We have now identified cluster-specific marker genes (i.e. genes that are uniquely upregulated in each cluster compared to all other clusters) and performed a focused analysis using Zoe's marker genes and the cyclin genes.
...
```{r libsize, fig.cap = "Library sizes of cells in each cluster."}
