...
Typically, we would build the graph using the MNN-corrected principal components so as to incorporate the batch effect correction.
However, in this context where batch effect is not orthogonal to the biology, we could only construct the graph using the uncorrected principal components.
...
We visually examine the clusters on a UMAP plot to confirm that a sensible partitioning was generated.
Figures \@ref(fig:clusterplot-umap) show that  cells from the day1 samples form a cluster with two subpopulations (i.e. cluster 6 and 7) distinctive from the rest. 
This actually go in line with the outcomes of our QC, where samples from day1 are slightly lower in count depth and features, but with higher level of variance compared to day3 and day6.
The clustering pattern may also indicate the unique cell-types or -state of the two clusters of cells in day1.
However, this pattern also match with cells from plate LCE297, implying it could partially or even completely be an outcome of plate-based batch effect.
Day3 and day6 harbors four and three clusters respectively, but with one cluster shared between the days (i.e. cluster 4).
This could correspond to cells at an interim state or developmental stage transiting from day 3 to 6.
Moreover, samples from the same day of collection are quite homogeneous among themselves, suggesting the uniformity among samples from each day.
...
# stopifnot(all(unlist(interesting["Cdcp1", -c(1:3)]) > 0))
```
...
Some inspection of Figures \@ref(fig:heat-logFC) and \@ref(fig:heat-logExp) suggest that cluster `r chosen` are distinguished by the relatively high expression level of the *different isoforms of histone H2A*. Also, the higher expression only observed in day3 and day6, in which expression at day6 are in general higher than day3 within the cluster.
Notably, expression of another 2 genes, *Gstm2*, and *Sparc*, are found to be higher in cluster 8 (which is a cluster mostly shared between cells from day1 and day6) and with a decreasing trend of expression through day1 to day6.
...
There are **TODO: HOW MANY (and describe)** murine reference data sets available:
...
We use `r Biocpkg("SingleR")` to annotate the data set against `MouseRNAseqData` as it is more likely to be relevant to the SSC from mouses.
...
Of note, the `MouseRNAseqData` contains various cell sub-types, even though it may not be highly relevant to murine SSC.
...
**TODO:** We'll discuss with Ruili whether it makes sense to analyse cell cycle for these cells. It will be somewhat challenging to interpret because these are such an unusual cell type (compared to the reference datasets of cell cycle gene expression). For now, I've included a few different analyses of cell cycle, but ultimately we may remove these entirely.
...
However, it is not difficult to find routine violations of this assumption&[see https://osca.bioconductor.org/cell-cycle-assignment.html#using-reference-profiles for an illustration].
