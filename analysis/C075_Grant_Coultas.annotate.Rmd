...
We start from the preprocessed *SingleCellExperiment* object created in ['Selection of biologically relevant cells for the Cooney (C057) memory CD4+ T-cell data set'](C075_Grant_Coultas.cell_selection.html).
...
In ['Selection of biologically relevant cells for the Cooney (C057) memory CD4+ T-cell data set'](C075_Grant_Coultas.cell_selection.html), we obtained one set of labels:
...
In this section we use pre-defined gene lists and construct *de novo* marker gene lists to help us interpret the clustering results.
...
The advantage of using the `reconstructed` values for visualization is that these will generally eliminate any visual differences between the batches (in this case, between the plates), which can result in 'cleaner' figures.
...
- Some of these marker genes are not cluster-specific (e.g., *Igfbp3*, *Egr1*, *Spock2*, *Ccna2*), which may mean that they are not cell type-specific.

<aside>
Violin plots of per-gene expression measurements (`logcounts` and `reconstructed`) can be found in the [`output/zoes_marker_genes/`](../output/zoes_marker_genes/) directory.
</aside>

```{r zoes-markers-heatmap, fig.wide = TRUE, fig.cap = "Heatmap of row-normalized `logcounts` (left) and `reconstructed` values (right) for Zoe's marker genes. Each column is a sample, each row a gene. Samples are ordered by `cluster` then by `genotype`. Row (gene) and column (cell) order are preserved across the two heatmaps.", fig.asp = 1.1}
...
CSV files of the cluster marker gene lists can be found in the [`output/cluster_marker_genes/`](../output/cluster_marker_genes/) directory.
...
The following table gives the number of marker genes for each cluster by this approach ($FDR < 0.05$).
...
knitr::kable(
  data.frame(
    ...
    n = sapply(markers, function(x) sum(x$FDR < 0.05))),
  caption = "Number of marker genes for each cluster. ")
```
...
For each marker gene strategy we provide a PDF of selected marker genes for each cluster in **TODO**: directory.
Here, we describe the details of the choices made in creating these heatmaps.
**TODO:** Summarise these choices and link to relevant subsection.

**TODO**: Figure out how to merge with the top-level 'Visualization' section; I think I just want to move the top-level section down here and incorporate it (making any PDFs in the relevant sections).

## Limitations of `t_up_all` strategy

As an example of the information available in these gene lists, Figure \@ref(fig:t-up-all-heatmap) is a heatmap showing the expression of the top-10 marker genes for each cluster from the [`t_up_all`] analysis. 
From this figure we can observe that:

- Most clusters only have a small number of marker genes, if any, by this approach.
- Cluster 1 has very high and distinct expression of several histone genes.

The lack of marker genes for some clusters under the `t_up_all` strategy necessitates the other strategies.

```{r t-up-all-heatmap, fig.wide = TRUE, fig.cap = "Heatmap of top-10 `t_up_all` marker genes using the row-normalized `logcounts (left) and the row-normalized `reconstructed values (right) for cells in each cluster. Each column is a sample, each row a gene. Note that some clusters have as few as zero upregulated cluster-specific marker genes. Row (gene) and column (cell) order are preserved across the two heatmaps.", fig.asp = 1.1}
...
Figure \@ref(fig:t-up-some-heatmap) is a heatmap showing the expression of the top-50 marker genes for cluster 2 from the [`t_up_some`] analysis. 
From this figure we can observe that:

- Many of the cluster 2 `t_up_some` marker genes are pseudogenes, with several others that are ribosomal protein genes and sex chromosome genes.
...
```{r t-up-some-cluster-2-heatmap, fig.wide = TRUE, fig.cap = "Heatmap of top-50 `t_up_all` marker genes for cluster 2 using the row-normalized `logcounts (left) and the row-normalized `reconstructed values (right) for cells in each cluster. Each column is a sample, each row a gene. Row (gene) and column (cell) order are preserved across the two heatmaps. Genes are annotated according to `gene_type`.", fig.asp = 1.1, eval = FALSE}
...
**TODO**: Re-write
We have now performed a focused analysis using Zoe's marker genes and the cyclin genes, and identified cluster marker genes for each cluster using severa strategies.

<aside>
Additional analyses focused on specific gene sets could be considered.
</aside>

**TODO:** Re-write summary
The cluster marker gene analysis analysis identified cluster `r g2_cluster` as having by far the largest number of cluster-specific marker genes.
Cells in this same cluster had high expression of genes associated with a different phase of the cell cycle (G2) to the majority of cells in other clusters (G1).
Figure \@ref(fig:libsize) shows that cells in this cluster also have larger library sizes than cells in other clusters.

```{r libsize, fig.cap = "Library sizes of cells in each cluster.", eval = FALSE}
plotColData(sce, "sum", x = "cluster", colour_by = "cluster") + 
  scale_fill_manual(values = cluster_colours) + 
  scale_y_log10() + 
  guides(FALSE)
```

However, based on the `r BiocStyle::Biocpkg("SingleR")` analysis, the cells in this cluster appear to be CD4+ T-cells, just like the other cells in the dataset and not of any unique subtype.

One possibility that is suggested by this set of results is that these are activated T-cells that are actively proliferating.

<aside>
It is worth noting that cells in this cluster are split roughly 50:50 between `Infected` and `Uninfected` samples. So this 'activation' signal does not appear to be associated with `Treatment`.
</aside>

These marker gene lists should be used in combination with the automated cell type labelling from `r BiocStyle::Biocpkg("SingleR")` to develop biologically relevant, meaningful, and useful labels for cells in this dataset.
...
- [ ] Create cluster-specific PDF heatmaps for the top-k set of marker genes (do actual computations in the relevant section) and link to 'Visualization' section.
  - [ ] Exclude pseudo, ribo, sex, and mito from marker gene heatmaps.
  - [ ] Perform row clustering.
