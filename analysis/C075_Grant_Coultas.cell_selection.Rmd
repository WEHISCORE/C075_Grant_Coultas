...
plot_grid(
  ggcells(sce, aes(x = UMAP.1, y = UMAP.2)) +
...
#   sce,
#   features = features,
#   colour_columns_by = c("batch", "cluster"),
#   center = TRUE,
#   symmetric = TRUE,
#   order_columns_by = c("batch", "cluster"),
#   cluster_rows = FALSE,
#   cluster_cols = FALSE,
#   annotation_row = data.frame(
#     markers = rep(levels(sce$batch), each = n),
#     row.names = features),
#   zlim = c(-4, 4))
...
**TODO**: UP TO HERE. Email Zoe re the clustering result. Then, proceed with excluding the smaller cluster (after checking it's the same plates we excluded last time) and continue with the analysis.
