...
spike_drop_subset <- union(spike_drop_subset, "LC398")
mito_drop <- isOutlier(
...
# NOTE: LC294 and LC358 are outliers for mito%. Zoe says she doesn't know of 
#       any good reason for this result. Furthermore, if we `subset` these
#       plates then almost all cells on these plates are excluded. Therefore, 
#       we don't `subset` these plates Instead, we will just keep an eye on
#       these cells in downstream analysis (and potentially exclude them).
...
ggplot(as.data.frame(colData(sce_pre_QC_outlier_removal))) + 
...
Strong eviations from the diagonal would correspond to composition biases due to differential expression between cell subpopulations.
...
As noted in @lun2017assessing, "[It] is more difficult to evaluate [absolute quantification because] accuracy is affected by the magnitude of the differences in the behavior of spike-in and endogenous transcripts.
...
```{r umap, fig.cap = "UMAP plot of the dataset. Each point represents a cell and is coloured by `plate_number`. Each panel highlights droplets from a particular `plate_number`.", fig.asp = 0.8}
...
- There are strong plate-specific differences (i.e. batch effects). In particular, plates `LC294` and `LC358` are very different from the majority of plates.
