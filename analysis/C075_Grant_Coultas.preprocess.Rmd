...
In particular, we keep the plate number (`plate_number`), well position (`well_position`), the sample type (`sample_type`), the genotype (`genotype`), and the mouse (`mouse`), and the sequencing run (`sequencing_run`).
...
genotype_colours <- setNames(
  RColorBrewer::brewer.pal(nlevels(sce$genotype), "Set1"),
  levels(sce$genotype))
...
**TODO:** Revisit the removal of LC279 after re-running scPipe with the correct sample sheet. I don't think there is any need to remove; that it only contains a control sample still makes it marginally useful.

Figure \@ref(fig:breakdown-by-plate-number) shows that each plate has cells from a knockout and a wildtype mouse.
Each pair of mice on a plate are siblings.
Mouse `#1824` acts as a control sample on two plates.
...
- `sum`: This measures the library size of the cells, which is the total sum of counts across both genes and spike-in transcripts. 
We want cells to have high library sizes as this means more RNA has been successfully captured during library preparation. 
...
Cells with few expressed features are likely to be of poor quality, as the diverse transcript population has not been successfull captured. 
- `altexps_ERCC_detected`: This measures the proportion of reads which are mapped to spike-in transcipts relative to the library size of each cell. 
...
There are some plates with a higher average percentage of reads coming from the ERCC spike-ins (LC294 and LC358) or with a subset of samples with notably larger values (LC279 and LC398).
...
```{r plate-map-ercc, fig.cap = "Percentage of reads coming from ERCC spike-ins for all single-cell samples in the dataset plotted by plate position. The colour scale runs from dark blue (0% of reads coming from ERCC spike-ins) to bright yellow (50% of reads coming from ERCC spike-ins)."}
...
Again, there is no systematic trend (**TODO**: Re-visit)
...
Due to the differences in the QC metrics by plate, we will perform our outlier removal at the plate-level.
...
The vast majority of cells are retained for most plates.
More cells are removed based on low library size and number of expressed genes than by high percentages of ERCC transcripts or mitochondrial RNA. 
...
**TODO:** Consider global cutoffs, too.
...
```{r qcplot-post-outlier-removal-by-plate-mouse, fig.cap = "Distribution of QC metrics for each plate in the dataset. Each point represents a cell and is colored according to whether it was discarded during the QC process.", layout = "l-page", fig.asp = 0.7}
...
**TODO:** Are these biologically relevant, discuss with Zoe.
...
```{r}
# TODO: Should I be using multiBatchNorm()?
...
- [x] Zoe to send me actual sample sheet for LC279.
  - We did sequence LC279 but I processed it with scPipe using what is actually the sample sheet for LC280. LC279 actually contains only control cells from a single mouse and should have a fair number of empty wells.
- [ ] Nominally empty wells on LC279 have reads; why?
- [ ] LC398 might be out of alignment, so interpret with care.
