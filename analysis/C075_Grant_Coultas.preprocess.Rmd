...
The plate-level summaries of selected QC metrics are given in the table below.

```{r}
dplyr::bind_rows(
  makePerCellDF(sce, exprs_values = "counts") %>%
  ...
  dplyr::summarise(
    ...
    median_detected = median(detected),
    mean_detected = mean(detected)) %>%
  dplyr::mutate(plate_number = "overall")) %>%
  knitr::kable(caption = "Summary of selected QC metrics.")
```

The distributions of these metrics are shown in Figure \@ref(fig:qcplot-by-plate-by-mouse), stratified by plate and mouse.

<aside>
A PDF version of Figure \@ref(fig:qcplot-by-plate-by-mouse) **but with LC294 and LC358 excluded and only showing the single cells** is available in [`output/qc_plots/qc_plots_before.pdf`](../output/qc_plots/qc_plots_before.pdf).
</aside>
...
```{r}
# NOTE: Zoe requested a PDF version of the 'before' QC plot but with LC294 and 
#       LC358 excluded (I interpreted this to also mean just showing the 
#       single cells).
sce_ <- sce[, !sce$plate_number %in% c("LC294", "LC358") & 
...
dir.create(here("output", "qc_plots"))
ggsave(
  here("output", "qc_plots", "qc_plots_before.pdf"),
  p,
  width = 7,
  height = 6)
```
...
```{r barplot-highlighting-outliers, fig.cap = "Cells removed during QC, stratified by `plate_number` and `genotype`. "}
...
<aside>
A PDF version of Figure \@ref(fig:qcplot-post-outlier-removal) **but with LC294 and LC358 excluded and only showing the single cells** is available in [`output/qc_plots/qc_plots_after.pdf`](../output/qc_plots/qc_plots_after.pdf).
</aside>
...
```{r}
# NOTE: Zoe requested a PDF version of the 'after' QC plot but with LC294 and 
#       LC358 excluded (I interpreted this to also mean just showing the 
#       single cells).
sce_ <- sce[, !sce$plate_number %in% c("LC294", "LC358") & 
              sce$sample_type == "single cell"]
...
ggsave(
  here("output", "qc_plots", "qc_plots_after.pdf"),
  p,
  width = 7,
  height = 6)
```
...
# Inspecting *Pglyrp1* expression

Zoe requested a violin plot of *Pglyrp1* expression in all cells, which is available as [`output/violin_plots/Pglyrp1.violin_plot.logcounts.pdf`](output/violin_plots/Pglyrp1.violin_plot.logcounts.pdf).

```{r, results = "hide"}
dir.create(here("output", "violin_plots"))
pdf(
  here("output", "violin_plots", "Pglyrp1.violin_plot.logcounts.pdf"), 
  height = 5,
  width = 5)
plotExpression(sce, "Pglyrp1", feature_colours = FALSE)
dev.off()
```
...
Zoe and Anne advised that pseudogenes, ribosomal protein genes, genes on the sex chromosomes, and mitochondrial genes are of lesser biological relevance to this study, so it was decided to exclude them from the HVGs.
...
Figure \@ref(fig:umap) visualizes the cells using the UMAP co-ordinates.

```{r umap, fig.cap = "UMAP plot of the dataset. Each point represents a cell and is coloured by `plate_number`. Each panel highlights cells from a particular combination of `genotype` and `sex`.", fig.asp = 0.8}
