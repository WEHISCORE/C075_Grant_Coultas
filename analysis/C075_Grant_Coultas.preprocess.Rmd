...
However, Zoe asked that we disregard samples from plate LC279 from further analysis^["Please disregard LC279 from any further analysis, it was not supposed to be sequenced and does not correspond to the plate layout you have (actually contains many empty wells, which is why the spike in RNA counts are so high). I am only interested in the other plates LC280, 294, 358, 392, 396, 398." - email from Zoe on 2020-01-07.]

```{r, eval = FALSE}
# TODO: Evaluate this chunk after checking claim that this plate has many high ERCC% wells
sce <- sce[, sce$plate_number != "LC279"]
...
**TODO:** Check with Zoe if these are all sib pairs.
...
p <- lapply(levels(sce$plate_number), function(p) {
  ...
  plotPlatePosition(
    ...
    by_show_single = TRUE) +
    ggtitle(p) +
    # TODO: Tweak limits (especially is LC279 is removed)
...
Again, there is no systematic trend (**TODO**: Apart from that on LC279)
