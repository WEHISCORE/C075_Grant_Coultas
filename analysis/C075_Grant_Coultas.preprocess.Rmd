...
# Removal of plates LC294 and LC358

**Based on some preliminary analysis, it was decided to remove plates LC294 and LC358 from the data set, at least for the time being.**

```{r}
sce <- sce[, !sce$plate_number %in% c("LC294", "LC358")]
...
# NOTE: Skipping this in order to preserve colours
...
```{r normplot, fig.cap = "Size factors from deconvolution, plotted against library sizes for all cells in each data set. Axes are shown on a log-scale.", results = "hide", fig.asp = 4 / 5}
...
We can empirically investigate the (1) whereas (2) is difficult to verify in practice.
We stress that we focus on *relative* normalization, i.e., to make counts com-
parable across cells.
...
As noted by [@lun2017assessing]:

> [It] is more difficult to evaluate [absolute quantification because] accuracy is affected by the magnitude of the differences in the behavior of spike-in and endogenous transcripts.
In contrast, relative normalization is only affected by variability in the differences in behavior across wells.

### Investigating spike-in levels
...
Figure \@ref(fig:spike-in-perc) shows spike-in transcripts as a percentage of the library size for each cell in our dataset compared to a the data from Lun (2017)[@lun2017assessing].

```{r spike-in-perc, fig.cap = "Percentage of reads coming from ERCC spike-in transcripts per sample. On the left is the current dataset and on the right is data from Lun (2017)."}
p1 <- plotColData(
  sce,
  x = "genotype",
  y = "altexps_ERCC_percent",
...
plot_grid(p1, p2, ncol = 2, align = "h", rel_widths = c(5, 2))
```

In ideal experiments, spike-in transcripts would take up 5%–10% of the library size for each cell [@lun2017assessing], which we have largely achieved.
However, Figure \@ref(fig:spike-in-total) shows that the absolute number of reads from the ERCC spike-in transcripts is some 100-times lower than that obtained in [@lun2017assessing].

```{r spike-in-total, fig.cap = "On the left is the current dataset and on the right is data from Lun (2017)."}
# TODO: Fix xaxis labels in p1
p1 <- plotColData(
  sce,
  x = "genotype",
  y = "altexps_ERCC_sum",
  ...
  facet_grid(~plate_number) +
  guides(fill = FALSE) +
  scale_y_log10(limits = c(1, max(sce.416b$altexps_ERCC_sum))) + 
  ggtitle("C075") + 
  ylab("Total ERCC read count") +
    annotation_logticks(
      sides = "l",
      short = unit(0.03, "cm"),
      mid = unit(0.06, "cm"),
...
**TODO** is ERCC coverage too low to be used? Not necessarily (comparable ERCC coverage to sce.richard).

```{r}
# TODO: This is sandpit stuff
# Supp figure 11 from https://genome.cshlp.org/content/suppl/2017/10/13/gr.222877.117.DC1/Supplemental_Materials.pdf
ercc <- data.table::fread("https://assets.thermofisher.com/TFS-Assets/LSG/manuals/cms_095046.txt")

par(mfrow = c(1, 2))
lapply(levels(sce.416b$block), function(b) {
  sce.416b <- sce.416b[, sce.416b$block == b]
  plot(
    ercc$`concentration in Mix 1 (attomoles/ul)`,
    rowMeans(counts(altExp(sce.416b))[ercc$`ERCC ID`, ]),
    log = "xy",
    xlab = "Theoretical",
    ylab = "Observed",
    main = paste0(
...
par(mfrow = c(2, 3))
lapply(levels(sce$plate_number), function(pn) {
  sce <- sce[, sce$plate_number == pn]
  ercc <- ercc[ercc$`ERCC ID` %in% rownames(altExp(sce))]
  plot(
    ercc$`concentration in Mix 1 (attomoles/ul)`,
    rowMeans(counts(altExp(sce))[ercc$`ERCC ID`, ]),
    log = "xy",
    xlab = "Theoretical",
    ylab = "Observed",
    main = paste0(
...
# Supp fig 12 from https://genome.cshlp.org/content/suppl/2017/10/13/gr.222877.117.DC1/Supplemental_Materials.pdf
boxplot(
  lapply(levels(sce.416b$block), function(b) {
    sce.416b <- sce.416b[, sce.416b$block == b]
    x <- rowMeans(counts(altExp(sce.416b)) + 1)
    log2(x)
  }),
  ylim = c(0, 20))
...
boxplot(
  lapply(levels(sce$plate_number), function(pn) {
    sce <- sce[, sce$plate_number == pn]
    x <- rowMeans(counts(altExp(sce)) + 1)
    log2(x)
  }))

# TODO: Compare deconvolution and spike in normalization by plate.
set.seed(666)
...
**TODO**: Re-write this paragraph (some is redundant with above) if pursuing this approach.
Practically, spike-in normalization should be used if differences in the total RNA content of individual cells are of interest and must be preserved in downstream analyses.
For a given cell, an increase in its overall amount of endogenous RNA will not increase its spike-in size factor.
This ensures that the effects of total RNA content on expression across the population will not be removed upon scaling. By comparison, the other normalization methods described above will simply interpret any change in total RNA content as part of the bias and remove it.

### Computing separate size factors for spike-in transcripts

**TODO:** Note that this happens regardless of the normalization strategy
...
**TODO:** A sentence saying which normalization factors we are using.
